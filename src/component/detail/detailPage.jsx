...
const DetailPage = ({data,changeNote,deleteNote}) => {
    return(
        <>
        <h3 className="detail-page__title">{data.title}</h3>
        <p className="detail-page__createdAt">{showFormattedDate(data.createdAt)}</p>
        <div className="detail-page__body">{data.body}</div>
        <div className="detail-page__action">
            <button className="action" type="button" title={data.archived ? 'Aktifkan' : 'Arsipkan'} onClick={() => changeNote(data.id,data.archived)}>
            ...
            <button className="action" type="button" title="Hapus" onClick={() => deleteNote(data.id)}>
